...
def fast_coverage_check(chromosome, data_matrix):
    """
    ...
    return np.all(np.any(data_matrix & chromosome, axis=1))


def detailed_coverage_check(chromosome, data_matrix):
    """
    ...
    counts = np.count_nonzero(data_matrix & chromosome, axis=1)
