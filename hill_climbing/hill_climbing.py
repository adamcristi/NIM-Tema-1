...
def next_ascent_hill_climbing(number_iterations, number_points, data_matrix, number_samples, number_candidates, total_used_sum):
        ...
        for index_current_point, current_point in enumerate(current_points):
            ...
            while modified_minimum:
                ...
                for index_neighbour, eval_neighbour in enumerate(evals_neighbours_current_point):
                    if eval_neighbour < evals_best_points[index_current_point]:
                        evals_best_points[index_current_point] = eval_neighbour
                        best_points[index_current_point] = neighbours_current_point[index_neighbour]
                        updated_current_point = neighbours_current_point[index_neighbour]
                        modified_minimum = True

                current_point = copy.deepcopy(updated_current_point)

    np.save(os.path.abspath(os.getcwd()) + '/hill_climbing/data_hill_climber/{}_{}_Instance_Best_Size_{}'.format(name.split('_')[0],
                                                                                                                 name.split('_')[1],
...
def steepest_ascent_hill_climbing(number_iterations, number_points, data_matrix, number_samples, number_candidates, total_used_sum):
    best_points = np.zeros((number_points, number_candidates), dtype=np.int64)
    evals_best_points = np.zeros((number_candidates,))
    ...
    for iteration in range(number_iterations):
        ...
        for index_current_point, current_point in enumerate(current_points):
            ...
            while modified_minimum:
                ...
                for index_neighbour, neighbour in enumerate(neighbours_current_point):
                    eval_neighbour = eval_point(point=neighbour, data_matrix=data_matrix, number_samples=number_samples,
                                                total_used_sum=total_used_sum)
                ...
                current_point = copy.deepcopy(updated_current_point)
                print(np.sum(current_point))

    np.save(os.path.abspath(os.getcwd()) + '/hill_climbing/data_hill_climber/{}_{}_Instance_First_Size_{}'.format(name.split('_')[0],
                                                                                                                  name.split('_')[1],
                                                                                                                  str(number_points)), best_points)
